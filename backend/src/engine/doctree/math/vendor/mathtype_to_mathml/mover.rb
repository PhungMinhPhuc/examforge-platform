  ...
  class Mover
    ...
    def move_following_subsup
      mathtype.xpath("//tmpl[(#{SUBSUP_SELECTOR}) and not(#{PRE})]").each do |el|
        ...
        if siblings.empty?
          # No preceding character/template to move into the slot.
        elsif siblings.last.xpath(CLOSE_PAREN)
    ...
    def move_preceding_subsup
      mathtype.xpath("//tmpl[(#{SUBSUP_SELECTOR}) and #{PRE}]").each do |el|
        ...
        if siblings.empty?
          # No following character/template to move into the slot.
        elsif siblings.first.xpath(OPEN_PAREN)
          siblings = siblings.reverse.take_while do |sibling|
